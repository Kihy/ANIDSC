Feature: Basic chaining of components
    ...
    Scenario: Process packets from an offline pcap reader
        Given a PacketReader initialized with dataset "../datasets/Test_Data" and file "malicious_Port_Scanning"
        And a feature extraction pipeline with AfterImage
        When the PacketReader starts
        Then the pipeline should not fail
        And the components are saved
    ...
    Scenario: Process packets from an offline pcap reader
        Given a PacketReader initialized with dataset "../datasets/Test_Data" and file "malicious_Service_Detection"
        And a feature extraction pipeline with AfterImage
        When the PacketReader starts
        Then the pipeline should not fail
        And the components are saved
