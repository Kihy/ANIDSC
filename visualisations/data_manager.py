...
class DataManager:
    ...
    @staticmethod
    def _set_progress(widget_mgr, visible: bool, active: bool = False, status: str = ""):
        if widget_mgr is None:
            return
        ...
        for i, line in enumerate(fh, start=1):
            ...
            if len(sample) < max_frames:
                ...
                if j <= max_frames:
                    sample[j - 1] = line
            if widget_mgr:
                widget_mgr.status.object = f"**{i:,} {data_type} processed...**"
    ...
    def load_frames(
        ...
        max_frames = max(1, widget_mgr.max_frames_input.value) if widget_mgr else 10_000
        if widget_mgr:
            start = max(0, widget_mgr.range_start.value or 0)
            end = max(start + 1, widget_mgr.range_end.value or float("inf"))
        ...
        self._set_progress(widget_mgr, visible=True, active=True, status="**Loading JSON frames...**")
        try:
            with fsspec.open(file_path, "rt", compression="zstd") as f:
                sample = self._reservoir_sample(f, max_frames, sample_range, widget_mgr, data_type="JSON frames")
            frames = [
            ...
            self._set_progress(widget_mgr, visible=False)

    def get_frame(self, file_path: Path, idx: int, widget_mgr=None) -> Optional[nx.Graph]:
    ...
    def count_frames(self, file_path: Path, widget_mgr=None) -> int:
        return sum(1 for _ in self.load_frames(file_path, widget_mgr))

    # ── CSV loading ──────────────────────────────────────────────────────────

    def read_csv(self, path: Path, widget_mgr=None) -> pd.DataFrame:
        ...
        if cache_key in self._csv_cache:
            ...
            self._set_progress(widget_mgr, visible=True, active=True, status="**Loading CSV data...**")
            try:
                with fsspec.open(path, "rt", compression="zstd") as f:
                    header = next(f)
                    sample = self._reservoir_sample(f, max_frames, sample_range, widget_mgr, data_type="CSV rows")
