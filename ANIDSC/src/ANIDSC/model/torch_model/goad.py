...
class GOAD(BaseTorchModel):
    ...
    def init_model(self):
        ...
        self.criterion = GoadLoss(
            alpha=self.alpha, margin=self.margin, device=self.device
        )
        self.optimizer = torch.optim.Adam(
            self.net.parameters(), lr=1e-3, weight_decay=1e-5
        )
    ...
    def forward(self, X, inference=False):
        ...
        if inference:
            ...
            self.rep_means = self.rep_means + (
                batch_rep.mean(0).unsqueeze(0) - self.rep_means
            ) / (self.total + 1)
            self.total += 1
    ...
    def goad_transforms(self, X):
        ...
        labels = (
            torch.arange(self.n_trans, device=self.device)
            .unsqueeze(0)
            .expand((X.shape[0], self.n_trans))
        )
        labels = labels.long()
...
class GoadNet(torch.nn.Module):
    def __init__(
        ...
        self.head = torch.nn.Sequential(
            ...
            torch.nn.Conv1d(n_hidden, n_output, kernel_size=kernel_size, bias=True),
        )
        return
...
class GoadLoss(torch.nn.Module):
    def __init__(self, alpha=0.1, margin=1.0, device="cuda"):
    ...
    def forward(self, rep, pred, labels):
        ...
        # Compute neg
        offset = (
            torch.eye(rep.size(1), device=rep.device).unsqueeze(0) * 1e6
        )  # Broadcast offset along the batch dimension
