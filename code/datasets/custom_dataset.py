...
class IterativeCSVDataset(torch.utils.data.Dataset):
    def __init__(self, feature_path, nb_samples, label_path=None, chunksize=1024, skip_rows=0):
...
def load_dataset(dataset_id, train_val_test=[0.8,0.1,0.1], batch_size=1024):
    ...
    if train_val_test == False:
        return IterativeCSVDataset(feature_path, total_rows, chunksize=batch_size)

    train_dataset=IterativeCSVDataset(feature_path, int(total_rows*train_val_test[0]), chunksize=batch_size)
    val_dataset=IterativeCSVDataset(feature_path, int(total_rows*train_val_test[1]), chunksize=batch_size, skip_rows=int(total_rows*train_val_test[0]))
    test_dataset=IterativeCSVDataset(feature_path, int(total_rows*train_val_test[2]), chunksize=batch_size, skip_rows=int(total_rows*sum(train_val_test[:2])))
