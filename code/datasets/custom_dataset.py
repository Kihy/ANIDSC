...
def load_dataset(dataset_id, train_val_test=[0.8, 0.1, 0.1], batch_size=1024):
    ...
    if train_val_test == False:
        return IterativeCSVDataset(
            dataset_id, feature_path, total_rows, chunksize=batch_size
        )

    train_dataset = IterativeCSVDataset(
        dataset_id + "_train",
        feature_path,
        int(total_rows * train_val_test[0]),
        chunksize=batch_size,
    )
    val_dataset = IterativeCSVDataset(
        dataset_id + "_val",
        feature_path,
        int(total_rows * train_val_test[1]),
        chunksize=batch_size,
        skip_rows=int(total_rows * train_val_test[0]),
    )
    test_dataset = IterativeCSVDataset(
        dataset_id + "_test",
        feature_path,
        int(total_rows * train_val_test[2]),
        chunksize=batch_size,
        skip_rows=int(total_rows * sum(train_val_test[:2])),
    )
