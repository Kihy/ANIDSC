...
class GOAD(BaseOnlineODModel,torch.nn.Module, TorchSaveMixin):
    ...
    def forward(self, X):
        ...
        if self.training:
            if self.rep_means is None:
                self.rep_means = batch_rep.mean(0).t()
            else:
                self.rep_means = self.rep_means+(batch_rep.mean(0).t()-self.rep_means)/self.nb
            self.nb += 1
    ...
    def process(self, X):
        ...
        # calc anomly score
        diffs = ((batch_rep.clone().unsqueeze(2) - self.rep_means.unsqueeze(0)) ** 2).sum(-1)
        diffs_eps = self.eps * torch.ones_like(diffs)
        diffs = torch.max(diffs, diffs_eps)
        logp_sz = torch.nn.functional.log_softmax(-diffs, dim=2)
