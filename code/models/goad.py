...
class GOAD(BaseDeepODModel,torch.nn.Module):
    """
    ...
    def __init__(self, epochs=100, batch_size=64, lr=1e-3,
        ...
        self.affine_weights = torch.from_numpy(np.random.randn(self.n_trans, self.n_features, self.trans_dim))
        self.net = GoadNet(
            self.trans_dim,
    ...
    def forward(self, X):
        ...
        if self.training:
            self.sum_reps += batch_rep.mean(0)
            self.nb += 1
    ...
    def goad_transforms(self, X):
        ...
        x_trans=x_trans.float().to(self.device)
    ...
    def on_train_begin(self):
        self.sum_reps = torch.zeros((self.hidden_dim, self.n_trans)).to(self.device)
        self.nb=0
        self.train()
    
    def on_train_end(self):
        reps = self.sum_reps.t() / self.nb
