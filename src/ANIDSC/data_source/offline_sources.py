...
class PacketReader(PipelineSource):
    ...
    def start(self):
        self.path = Path(
            f"{self.dataset_name}/pcap/{self.file_name}.pcap"
        )
...
class CSVReader(PipelineSource):
    def __init__(self, dataset_name:str, fe_name:str, file_name:str, 
                 n_features:int, max_pkts:int = float('inf'),
                 batch_size=1024):
        self.dataset_name=dataset_name 
        self.file_name=file_name
    ...
    def start(self):
        self.path = Path(
            f"{self.dataset_name}/{self.fe_name}/features/{self.file_name}.csv"
        )
