    ...
    def train(self):
        for loader in self.train_data:
            for feature, label in tqdm(
                loader,
                desc=f"training {self.model.model_name} on {loader.dataset.name}",
                leave=False,
            ):
...
class OutlierDetectionTrainer(BasePipeline):
    ...
    def train(self):
        if self.log_results:
            self.writer = SummaryWriter(
                f"runs/{self.dataset_name}/{self.model.model_name}"
            )
            ...
            if self.profile:
                prof = torch.profiler.profile(
                    ...
                    on_trace_ready=torch.profiler.tensorboard_trace_handler(
                        f"logs/{self.dataset_name}/{self.model.model_name}"
                    ),
                    record_shapes=False,
                    with_stack=False,
                )
            ...
            for loader in self.train_data:
                ...
                if self.log_results:
                    self.writer.add_scalar(
                        f"training loss {loader.dataset.name}",
                        np.mean(np.hstack(loss)),
                        i,
                    )
                    self.writer.add_scalar(
                        f"training time {loader.dataset.name}",
    ...
    def calc_threshold(self, i=0):
        ...
        for loader in tqdm(self.val_data):
            for feature, label in tqdm(
                loader,
                desc=f"calc threshold {self.model.model_name} on {loader.dataset.name}",
                leave=False,
            ):
                scores.append(self.model.predict_scores(feature))
                if self.log_results:
                    self.writer.add_scalar(
                        f"val scores {loader.dataset.name}",
                        np.mean(np.hstack(scores)),
                        i,
                    )
            loader.dataset.reset()
...
class OutlierDetectionEvaluator(BasePipeline):
    ...
    def eval(self):
        ...
        for loader in self.test_data:
            ...
            start_time = time.time()
            for feature, label in tqdm(
                loader, desc=f"evaluating {loader.dataset.name}"
            ):
                scores.append(self.model.predict_scores(feature))
            ...
            for metric in self.metrics:
                self.results[self.model.model_name][loader.dataset.file_name][
                    metric.__name__
                ] = metric(scores, self.model.threshold)

                if self.plot:
                    self.plot_scores(
                        scores,
                        self.model.threshold,
                        f"../datasets/{loader.dataset.dataset_name}/{loader.dataset.fe_name}/plots/{loader.dataset.file_name}/{self.model.model_name}.png",
                    )
