...
def name_map_func(value):
    models = ["VAE", "AE", "SLAD", "GOAD", "ICL", "KitNET", "BoxPlot", "ARCUS","Kitsune"]
    ...
    def plot_scores(self, df, dataset, file, filename):
        ...
        if self.col is None:
            g.axes[0, 0].fill_between(
                df["index"],
                df["lower_quartile_score"],
                df["upper_quartile_score"],
                alpha=0.5,
                color="#7FB7BE",
            )
            g.axes[0, 0].fill_between(
            ...
            for ax, col_name in zip(g.axes.flat, g.col_names):
                ...
                ax.fill_between(
                    col_data["index"],
                    col_data["lower_quartile_score"],
                    col_data["upper_quartile_score"],
                    alpha=0.2,
                )
        ...
        for dataset in self.datasets:
            ...
            for file in self.files:
                ...
                for filename in os.listdir(dir):
                    ...
                    if self.col is not None:
                        ...
                        df_summary = pd.DataFrame(df.agg(self.agg_funcs)).T
            ...
            if self.calc_f1:
                ...
                for pipeline, group in all_df.groupby("pipeline"):
                    ...
                    data["time"].append(group["time"].median())
            ...
            g = sns.catplot(
                data=all_df, kind="bar", x="pipeline", y="time", col=self.col
            )
            g.set_xticklabels(rotation=90)
