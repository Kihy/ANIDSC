...
def make_evaluator(**kwargs) -> dict:
    return {"evaluator": {"class": "BaseEvaluator"}}


def make_splitter(manifest, split_keys,name,  **kwargs):
    return {"pipeline":{"class": "MultilayerSplitter", "attrs":{"manifest":{key: manifest for key in split_keys}, "name":name, "protocol_map": {key: i for i, key in enumerate(split_keys)}}}}

def make_node_encoder(node_encoder, n_features, **kwargs):
    return {"model.node_encoder": {"class": node_encoder, "attrs":{"n_features":n_features}}}
...
def dict_to_yaml(pipeline_dict):
    return yaml.safe_dump(pipeline_dict, sort_keys=False, default_flow_style=False, indent=2)
...
def get_template(template_name, **kwargs):
    if template_name == "feature_extraction":
        ...
        components.update(make_evaluator(**kwargs))
        pipeline= make_pipeline(components)
        ...
        inner_components.update(make_evaluator(**kwargs))
